...
import RestauarantEditor from "../../components/add-new-post/EditorStock";
...
import SelectCityRegion from "../../components/add-new-post/restaurant/SidebarCityRegion";
import Logo from "../../components/add-new-post/logo";
import RestauarantMenu from "../../components/add-new-post/menu/menu";
...
class AddNewRestaurant extends React.PureComponent {
  // get static context
  ...
  setWholeContextStates = () => {
    ...
    /*
     * Update UI for selected City and Region
     * 1 - cityIdFK 2 - deliverableRegions
     * it should never be empty but still checking
     * console.log(`%c ${singleRestaurantData.cityIdFK}`, "background:white;color:red");
     * console.log(`%c ${singleRestaurantData.cityIdFK === undefined}`, "background:white;color:red");
     */
    if (singleRestaurantData.cityIdFK !== undefined)
      this.cityRegionRef.current.remoteSelectCityRegion(
        singleRestaurantData.cityIdFK,
        singleRestaurantData.mainCityObject,
        singleRestaurantData.deliverableRegions
      );
    // For city and regions
    // updating the context seperately inside there own components
  };
    ...
    return (
      <Container fluid className="main-content-container px-4 pb-4">
        ...
        <Row noGutters className="page-header py-4" style={{ justifyContent: "space-between" }}>
          ...
          />
          {/* {this.props.routeParams.isEditRestaurant && ( */}
          <Button
            theme="white"
            className="px-2"
            onClick={() =>
          ...
          <RestaurantContext.Consumer>
            {context => (
              <Row>
                ...
                <Col lg="9" md="12">
                  {/* Restauarant Info */}
                  <RestauarantEditor ref={this.editorRef} templateType={this.props.templateType} />
                  {/* TEST */}
                  {/* <pre style={{ maxHeight: "300px" }}>
                    {JSON.stringify(this.state.restaurantContext, null, 2)}
                  </pre> */}
                  {/* City and Region */}
                  <SelectCityRegion ref={this.cityRegionRef} restaurantContext={context} />
                  {/* Restauarant Menu Management */}
                  <RestauarantMenu
                    ref={this.restauarantMenuRef}
                  ...
                  <React.Fragment>
                    ...
                    <Logo
                      ...
                      onSaveLogoEvent={file => {
                        ...
                        context.setLogo(file);
                      }}
                      resetLogoContext={() => context.resetLogo()}
                    />
                  </React.Fragment>
                </Col>
              </Row>
