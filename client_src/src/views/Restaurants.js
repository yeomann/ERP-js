...
import { Container, Row, Col, Card, CardHeader, CardBody, Button, Badge } from "shards-react";
// components
...
export class Restaurants extends PureComponent {
  ...
  renderRestaurants(loader, restaurants) {
    if (!loader && Object.keys(restaurants).length !== 0) {
      ...
      return (
        <tr>
          <td className="p-3">
            No Restaurant Found. <Link to="/create-order">Click here to add New Restaurant</Link>
          </td>
        </tr>
      );
    }
  ...
  render() {
    ...
    const restNotFound = !this.props.restLoader && Object.keys(this.props.restaurants).length === 0;
    return (
      <div>
        <Container fluid className="main-content-container px-4">
          ...
          <Row noGutters className="page-header py-4 d-flex justify-between">
            ...
            />
            <Button squared theme="info" size="lg" onClick={() => history.push("/create-order")}>
              Add Restaurant
            </Button>
          </Row>
          {/* Default Light Table */}
              ...
              <Loader loaded={!deleteLoader}>
                <Card small className="mb-4">
                  <CardHeader className="d-flex justify-between">
                    <span className="m-0">
                      {restNotFound ? (
                        <Link to="/create-order">Add Restaurant</Link>
                      ) : (
                        "Quick edit by Click on any Restaurant"
                      )}
                    </span>
                  </CardHeader>
                  <CardBody className="p-0 pb-3">
                    <table className="table mb-0 restaurant-table">
                      ...
                      <tbody>{this.renderRestaurants(loader, allRestaurants)}</tbody>
                    </table>
                  </CardBody>
                </Card>
              </Loader>
